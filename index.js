  ...
  fetch(apiRequest)
    ...
    .catch((err) => {
      ...
      document.querySelector("main").innerHTML =
        '<img src="logo.svg" alt="" class="logo"><form><label for="location"><h2>Enter a valid location!</h2></label><select id="unit"><option value="metric">Metric</option><option value="us">US</option></select><div class="search"><input type="text" id="location"><button onclick="search()">Search</button></div></form>';
    })
    .then((res) => {
...
function shortTermForecast(res, unit) {
  ...
  document.querySelector(
    "#wind"
  ).innerText += ` ${res.currentConditions.windspeed} ${unit[1]}`;
...
function longTermForecast(res, unit) {
  document.querySelector("main").innerHTML =
    '<img src="logo.svg" alt=""><div class="forecast"><h2>Show short-term forecast</h2></div>';

  for (let i = 0; i < 15; i++) {
    document.querySelector("main").innerHTML += `<div id='day${
      i + 1
    }' class='days'></div>`;
    document.querySelector(`#day${i + 1}`).innerHTML += `<h3>Day ${
      i + 1
    }</h3><div>${res.days[i].datetime}</div><div>${res.days[i].temp} ${
      unit[0]
    }</div><div>Windspeed ${res.days[i].windspeed} ${unit[1]}</div><div>${
      res.days[i].uvindex
    } UV</div><div>${res.days[i].cloudcover}</div><div id='conditions'>${
      res.days[i].conditions
    }</div>`;
  }
